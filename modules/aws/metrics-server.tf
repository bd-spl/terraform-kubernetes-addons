...
module "helm_release_ecr_prepare_metrics-server" {
  source                = "./helm_release_ecr_prepare"
  helm_dependencies     = [for _, d in local.helm_dependencies : d if d.name == "metrics-server"]
  containers_versions   = local.metrics-server["containers_versions"]
  count                 = local.metrics-server["enabled"] ? 1 : 0
  repository            = local.metrics-server["repository"]
  name                  = local.metrics-server["name"]
  chart                 = local.metrics-server["chart"]
  chart_version         = local.metrics-server["chart_version"]

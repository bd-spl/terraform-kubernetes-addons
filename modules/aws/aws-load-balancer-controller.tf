...
module "helm_release_ecr_prepare_aws-load-balancer-controller" {
  source                = "./helm_release_ecr_prepare"
  helm_dependencies     = [for _, d in local.helm_dependencies : d if d.name == "aws-load-balancer-controller"]

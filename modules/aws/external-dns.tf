...
module "helm_release_ecr_prepare_external-dns" {
  source                = "./helm_release_ecr_prepare"
  helm_dependencies     = [for _, d in local.helm_dependencies : d if d.name == "external-dns"]
  containers_versions   = local.external-dns["containers_versions"]
  for_each              = { for k, v in local.external-dns : k => v if v["enabled"] }

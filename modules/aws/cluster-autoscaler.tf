...
module "helm_release_ecr_prepare_cluster-autoscaler" {
  ...
  helm_dependencies     = [for _, d in local.helm_dependencies : d if d.name == "cluster-autoscaler"]
  containers_versions   = local.cluster-autoscaler["containers_versions"]
  count                 = local.cluster-autoscaler["enabled"] ? 1 : 0
  repository            = local.cluster-autoscaler["repository"]
  name                  = local.cluster-autoscaler["name"]
  chart                 = local.cluster-autoscaler["chart"]
  chart_version         = local.cluster-autoscaler["chart_version"]

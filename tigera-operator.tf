locals {
  tigera-operator = merge(
    local.helm_defaults,
    {
      ...
      namespace              = try(local.helm_dependencies[index(local.helm_dependencies.*.name, "tigera-operator")].namespace, "tigera") #https://github.com/projectcalico/calico/issues/4812
      create_ns              = true
      enabled                = false
